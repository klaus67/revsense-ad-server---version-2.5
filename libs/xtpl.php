...
var $sub_blocks=array();						/* s#tore sub-block names for fast resetting */
...
function XTemplate ($file="", $contents="") {
	$this->mainblock="main";
	if($file <> "MEM")
...
function parse ($bname) {
	...
	// Parse the HTML -- Added by Ericson Smith: March 26, 2003 
	$copy = str_replace($searcher, $replacer, $copy);
...
function maketree($con,$block) {
	...
	while(list($k,$v)=each($con2)) {
		...
		#if (eregi($patt,$v,$res)) {
		$regex ='/(BEGIN:|END:)\s+([0-9a-zA-Z\_]+)\s+\-\->(.*)$/im';
		$v = str_replace("\n", "^^^^", $v);
		if(preg_match($regex, $v, $res)) {
			// $res[1] = BEGIN or END
			// $res[2] = block name
			// $res[3] = kinda content
